...
#
#     # Update particles
#     new_particles = validate_state(new_particles, bounds, d_mnt)
...
def update(samples, measurements, measurements_noise, forward_motion, angular_motion, process_noise, bounds, beta, resampling_threshold):
    samples = propagate_sample(samples, forward_motion, angular_motion, process_noise, bounds)

    d_mnt, likelihood = compute_likelihood(samples, measurements, measurements_noise, beta)
    samples = validate_state(samples, bounds, d_mnt)

    # Combine the importance weights
    weights = samples[0] * likelihood
    ...
    if N_eff < resampling_threshold*n_particles:
        resampler = Resampler(normalized_weights)
...
if __name__ == '__main__':
    ...
    for i in r:
        ...
        particles = update(particles, measurements, measurements_noise, robot_forward_motion, robot_angular_motion, process_noise, bounds, beta = 0.1, resampling_threshold = resampling_threshold)
