...
if apply_modif:
    ...
    # #Apply mask on MBES
    # mask_mbes = np.abs(np.diff(MBES_Z) - np.mean(np.diff(MBES_Z))) > 0.1*np.std(np.diff(MBES_Z))
    # print("Total point of MBES taken = ",MBES_Z[:-1,][~mask_mbes].shape[0]/MBES_Z.shape[0]*100,"%")
    # MBES_T = MBES_T[:-1,][~mask_mbes]
    # MBES_X = MBES_X[:-1,][~mask_mbes]
    # MBES_Y = MBES_Y[:-1,][~mask_mbes]
    # MBES_Z = MBES_Z[:-1,][~mask_mbes]

    # mask_mbes = np.abs(MBES_Z - np.mean(MBES_Z)) > 2*np.std(MBES_Z)
    # print("Total point of MBES taken = ",MBES_Z[~mask_mbes].shape[0]/MBES_Z.shape[0]*100,"%")
    # MBES_T = MBES_T[~mask_mbes]
    # MBES_X = MBES_X[~mask_mbes]
    # MBES_Y = MBES_Y[~mask_mbes]
    # MBES_Z = MBES_Z[~mask_mbes]
...
# dt_br = 0.2 #np.mean(np.diff(MBES_T))
# dt_br = 0.05 #ins

print(f"dt chosen = {dt_br}")
