...
using_offset = False # str(input("Using offset ? [Y/]")) == "Y"
...
def compute_likelihood(propagated_states, measurements, measurements_noise, beta, z_particules_mnt):
    ...
    if using_offset :
        d_MBES_mid_particule = new_z_particules_mnt
        ...
        mbes_min_Z, mbes_mid_Z, mbes_max_Z, \
            dp_x_mid, dp_y_mid, dp_x_min, dp_y_min, dp_x_max, dp_y_max,
            dp_x_mid_prev, dp_y_mid_prev, dp_x_min_prev, dp_y_min_prev, dp_x_max_prev, dp_y_max_prev = \
                measurements.flatten()
        ...
        _, d_mbes_particule_min_prev = distance_to_bottom(np.hstack((previous_state[0]+dp_x_min_prev,previous_state[1]+dp_y_min_prev)),MNT)
        _, d_mbes_particule_mid_prev = distance_to_bottom(np.hstack((previous_state[0]+dp_x_mid_prev,previous_state[1]+dp_y_mid_prev)),MNT)
        # _, d_mbes_particule_mid_prev = distance_to_bottom(np.hstack((previous_state[0],previous_state[1])),MNT)
        _, d_mbes_particule_max_prev = distance_to_bottom(np.hstack((previous_state[0]+dp_x_max_prev,previous_state[1]+dp_y_max_prev)),MNT)
...
def f_measurements(i, previous_measurements):
    ...
    dp_x_mid_prev = MBES_mid_Z[previous_ct_mbes,]/np.tan(angle_mid*np.pi/180)*np.cos(YAW[i,])
    dp_y_mid_prev = MBES_mid_Z[previous_ct_mbes,]/np.tan(angle_mid*np.pi/180)*np.sin(YAW[i,])

    dp_x_min_prev = -MBES_min_Z[previous_ct_mbes,]/np.tan(angle_min*np.pi/180)*np.cos(YAW[i,]-3*np.pi/2)
    dp_y_min_prev = -MBES_min_Z[previous_ct_mbes,]/np.tan(angle_min*np.pi/180)*np.sin(YAW[i,]-3*np.pi/2)

    dp_x_max_prev = MBES_max_Z[previous_ct_mbes,]/np.tan(angle_max*np.pi/180)*np.cos(YAW[i,]-np.pi/2)
    dp_y_max_prev = MBES_max_Z[previous_ct_mbes,]/np.tan(angle_max*np.pi/180)*np.sin(YAW[i,]-np.pi/2)

    measurements = np.array([MBES_min_Z[ct_mbes,] - MBES_min_Z[previous_ct_mbes,], MBES_mid_Z[ct_mbes,] - MBES_mid_Z[previous_ct_mbes,], MBES_max_Z[ct_mbes,] - MBES_max_Z[previous_ct_mbes,], \
                            dp_x_mid, dp_y_mid, dp_x_min, dp_y_min, dp_x_max, dp_y_max,\
                            dp_x_mid_prev, dp_y_mid_prev, dp_x_min_prev, dp_y_min_prev, dp_x_max_prev, dp_y_max_prev])
