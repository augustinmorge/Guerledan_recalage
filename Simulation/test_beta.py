...
if __name__ == '__main__':
    ...
    BETA = [1/(10**i) for i in np.arange(1,2.5,0.01)]
    ...
    for n_particles in N_PARTICULES:
        ...
        for k in tqdm(range(len(BETA))):
            ...
            file.write(str(np.mean(np.array(ERR)))+"; "+str(BETA[k])+"\n")
            file.flush()
        print(BETA, ERROR)

        # plt.figure()
        # plt.scatter(np.log(BETA)/np.log(10), ERROR)
        ...
        # f.savefig("./test_beta/Test_with_nb_part={}.png".format(n_particles))
    file.close()
